    ...
    return queryInterface.sequelize.transaction((t) => {
      return Promise.all([
        queryInterface.removeConstraint(
          "alert",
          "alert(user_id)-users(id)_fk",
          { transaction: t }
        ),
        queryInterface.removeConstraint(
          "livechats",
          "livechats(session_id)-livechat_session(id)_fk",
          { transaction: t }
        ),
        queryInterface.removeConstraint(
          "transactions",
          "transactions(user_id)-users(id)_fk",
          { transaction: t }
        ),
        queryInterface.removeConstraint(
          "transactions",
          "transactions(wallet_id)-wallets(id)_fk",
          { transaction: t }
        ),
        queryInterface.removeConstraint(
          "transactions",
          "transactions(beneficiary_id)-users(id)_fk",
          { transaction: t }
        ),
        queryInterface.removeConstraint(
          "transactions",
          "transactions(bank_id)-banks(id)_fk",
          { transaction: t }
        ),
        queryInterface.removeConstraint(
          "verification_tokens",
          "verification_tokens(user_id)-users(id)_fk",
          { transaction: t }
        ),
        queryInterface.removeConstraint(
          "wallet_balances",
          "wallet_balances(user_id)-users(id)_fk",
          { transaction: t }
        ),
        queryInterface.removeConstraint(
          "wallet_balances",
          "wallet_balances(wallet_id)-wallets(id)_fk",
          { transaction: t }
        ),
      ]);
    });
