...
import { AnimatedBeam } from "./animated_beam"
...
export default function AnimatedBeamMultipleOutputDemo({
    ...
    // Define your circle configurations
    const circleConfigs = React.useMemo(() =>
 
        [...wallets.map((e) => {
            return { icon: <img src={e.icon} alt={e.name} className=" rounded-full" />, group: 'outputs' };
        }),
        { icon: <Logo className={''} />, group: 'center', size: 'size-16' },],
        []);

    // Create refs array matching the number of circles
    const circleRefs = useRef(circleConfigs.map(() => useRef<HTMLDivElement>(null)));
    ...
    return (
        ...
        >
            <div className="flex size-full max-w-lg flex-row items-stretch justify-between gap-10">
                {/* Input Circle (if exists) */}
                {circleConfigs.some(c => c.group === 'input') && (
            ...
            {/* Animated Beams */}
            {circleConfigs.map((fromConfig, fromIndex) => {
                if (fromConfig.group === 'outputs') {
                    const centerIndex = circleConfigs.findIndex(c => c.group === 'center');
                    return (
                        <AnimatedBeam
                            key={`beam-${fromIndex}`}
                            containerRef={containerRef}
                            fromRef={circleRefs.current[fromIndex]}
                            toRef={circleRefs.current[centerIndex]}
                            duration={3}
                        />
                    );
                }
                if (fromConfig.group === 'center') {
                    const inputIndex = circleConfigs.findIndex(c => c.group === 'input');
                    if (inputIndex !== -1) {
                        return (
                            <AnimatedBeam
                                key={`beam-${fromIndex}`}
                                containerRef={containerRef}
                                fromRef={circleRefs.current[fromIndex]}
                                toRef={circleRefs.current[inputIndex]}
                                duration={3}
                            />
                        );
                    }
