...
import { Button, Callout, Card, Dialog, IconButton, Text } from "@radix-ui/themes"
...
export function SendMoneyModal({
    ...
    const [bankTransferMethod, setBankTransferMethod] = useState<'wire' | 'ACH' | 'internal'>('wire');
    ...
    return <>
        <Dialog.Root open={isOpen} onOpenChange={(value) => {
            ...
            <Dialog.Content className="mobile:w-fit tablet:min-w-[300px] tablet:w-[500px]">
                    ...
                    <>
                        ...
                        <div className="flex flex-col tablet:flex-col tablet:gap-5 gap-5 mt-5">
                            <div className="w-full">
                                <Text size={'2'} trim={'end'} as="div" weight={'bold'} className="mb-2">From</Text>
                                <ConvertInputCard
                                    inputProps={{
                                        id: 'amount_input',
                                        type: "number",
                                ...
                                {sendFrom.type == 'fiat' && (<div>
                                    ...
                                    <div className="mt-2 text-purple-500 text-[10px] leading-normal">
                                        {bankTransferMethod == "ACH" && "An affordable and reliable option for transferring money between U.S. bank accounts, typically processed within 1-3 business days."}
                                        {bankTransferMethod == "wire" && "A fast and secure option for sending money domestically or internationally, with domestic transfers usually processed the same day and international transfers taking 1-2 business days."}
                                ...
                                <div className="mt-5">
                                    <Text size={'2'} weight={'bold'} className="mt-7 mb-2">To</Text>

                                    {sendFrom.type == "crypto" && <div>
                                        <form onSubmit={handleSubmit2(submit)}>
                                            <Card className="pb-2 pt-3">
                                                <div className="flex flex-row items-center gap-2">
                                                    ...
                                                    <input
                                                        ...
                                                        type="text" className="mb-[2px] text-[14px] select-none text-wrap items-end bg-transparent shadow-none w-full outline-0"
                                                    ...
                                                    }} variant="ghost" color="gray" radius="full"><ClipboardPaste size={15} /></IconButton>
                                                </div>
                                                {sendFromWallet?.wallet?.network && <Text className="text-[10px] text-yellow-500">Ensure you're transferring to a wallet with {sendFromWallet?.wallet?.network} network to avoid assets lost.</Text>}
                                            </Card>
                                            ...
                                            <div className="w-full">
                                                ...
                                                <Button loading={is_submitting2} size={'3'} type="submit" className="mt-5 w-full">Submit</Button>
                                            </div>
                                        </form>
                                    </div>}
                                                        ...
                                                        {sendFrom.type == "fiat" && <>
                                                            <div>
                                                                <Text size={'1'} color="gray">
                                                                    {!bank_watch()?.account_name ? "Click on edit to fill withdrawal account details" : <>
                                                    ...
                                                    <Dialog.Root open={isOpen2} onOpenChange={(value) => {
                                                        ...
                                                        <Dialog.Content>
                                                            ...
                                                            <form className="gap-2 flex flex-col" onSubmit={sendFrom.type == "fiat" ? handleSubmit(() => {
                                                                ...
                                                                <Button loading={is_submitting1} size={'3'} type="submit" className="mt-5 w-full">Save</Button>
                                                            </form>
                                                        </Dialog.Content>
                                                    </Dialog.Root>
                                                </div>
                ...
                {isSuccessful && <div className="flex flex-col items-center">
                    <CheckCheck size={100} className="text-primary-500 mb-3" />
                    <Text size={'5'} weight={'bold'}>Withdrawal Successful</Text>
                    <Text align={'center'} size={'1'} color="gray">{isSuccessful}</Text>
                    <Button variant="ghost" color="red" className="w-[200px] mt-5" onClick={() => {
                        setIsOpen(false);
                    }}>Close</Button>
                </div>}
            </Dialog.Content >
