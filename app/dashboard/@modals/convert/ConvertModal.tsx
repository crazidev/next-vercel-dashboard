...
export default function ConvertModal({
    ...
    if (convert?.convert?.isReady == true) {
      if (state.from?.type == "fiat" || state.from?.type == "crypto") {
        if (activeInput == "input_1") {
          _inputFiat = convert.convert[state.from?.shortName]["USD"](
            state.input
          );
        } else {
      ...
      if (state.to?.type == "fiat" || state.to?.type == "crypto") {
        if (activeInput == "input_1") {
          _convertedAmount = convert.convert[state.from?.shortName][
            state.to?.shortName
          ](state.input - fee);
          _convertFiat =
            convert.convert[state.to?.shortName]["USD"](_convertedAmount);
        } else {
          _convertedAmount = convert.convert[state.to?.shortName][
            state.from?.shortName
          ](state.input - fee);
          _convertFiat =
            convert.convert[state.from?.shortName]["USD"](_convertedAmount);
        }
  ...
  function setWalletList() {
    ...
    setState((prev) => ({
      ...
      dropDownTo: [main(state?.to?.value), ...dropdownTo].filter(
        (e) => e.value != state.from?.value
      ),
...

      {
        <Dialog.Content className="mobile:w-fit tablet:min-w-[500px] tablet:w-[500px] lg:min-w-[800px]">
          ...
          <Text
            ...
            className=" mb-5"
          >
            Convert funds between wallets. Select the source wallet, target
            wallet, and amount for a seamless transfer.
          </Text>
          ...
          {state.loading ? (
            <Spinner className="m-auto my-[50px]" />
          ) : (
              ...
              <div className="tablet:w-full mobile:w-full w-[50%]">
                <ConvertInputCard
                  ...
                  currency={{
                    ...
                    onSelect: (v) => {
                      var to =
                        state.to?.value == v?.value ? state?.from : state.to;
                      setState((prev) => ({
                        ...prev,
                        from: v,
                      }));
                ...
                <div className="flex flex-row justify-between">
                  ...
                  <Text trim={"both"} size={"1"} color="gray">
                    Available Balance:{" "}
                    <b>
                      {cFmt({
                        amount:
                          state?.selectFrom?.balance ??
                          user?.accountBalance ??
                          0,
                      })}{" "}
                      {state?.selectFrom?.wallet?.shortName}
                    </b>
                    .
                  </Text>
                </div>
                    ...
                    {canConvert() && (
                      <div className="flex justify-between">
                        ...
                        <Text
                          ...
                          className="font-bold flex flex-row items-center gap-3"
                        >
                          1 {state?.from?.shortName} ≈{" "}
                          {convert.convert[state?.from?.shortName ?? "USD"][
                            state?.to?.shortName ?? "USD"
                          ](1)}{" "}
                          {state.to?.shortName}
                        </Text>
                      </div>
                ...
                <>
                  <div className="tablet:w-full mobile:w-full flex-grow pointer-events-none cursor-none">
                    <MiniChart
                      ...
                      isTransparent
                      height={
                        layout.isMobile ? 200 : layout.isTablet ? 250 : 350
                      }
                      width="100%"
                    ></MiniChart>
                  </div>
